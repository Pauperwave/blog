...
`server/database/cards.db` is generated by `bun run download-cards` (`scripts/download-bulk-data.ts`) — it needs to exist before `dev`/`build` or the transformers fall back to slower per-card Scryfall API calls. Despite the name, it's **committed to git** (only the intermediate `server/database/oracle-cards.json` bulk-data dump is gitignored), so regenerate and commit it after Pauper legality changes rather than assuming it's a local-only artifact.
