...
- **Contesto (2026-07-24):** setup dell'autenticazione GitHub OAuth per `/editor` (`docs/BACKLOG.md` #1), poi scoperta di un'incompatibilità architetturale più profonda che ridimensiona l'utilità di Nuxt Studio per questo progetto.
...
- **Decisione (parte 2):** Nuxt Studio resta configurato e autenticato correttamente, ma il suo utilizzo pratico è ridimensionato — probabilmente utilizzabile solo per contenuti "puri" senza i tre shortcode custom. `docs/BACKLOG.md` #5 (editor custom) è stato ripromosso da P3 a P2 di conseguenza: un editor su misura che lavora sul markdown grezzo pre-transform (invece che sul renderizzato) è probabilmente necessario, non ridondante. Esplorazione in corso, vedi Backlog #5 per il progetto di riferimento (`nuxt-ui-templates/editor`).
...
- **Decisione finale (utente, 2026-07-24):** procedere con un editor custom basato su TipTap (via `UEditor` di `@nuxt/ui`, già disponibile nella versione installata `4.9.0` — nessun bump richiesto) invece di continuare a investire su Nuxt Studio. Vedi `docs/BACKLOG.md` #5 per il piano.
