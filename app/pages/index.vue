...
const { data: posts } = await useAsyncData('articles', () => queryCollection('articles').all())
</script>
    ...
    <div class="flex justify-center">
      <ul class="space-y-4 mt-6 w-full max-w-2xl">
        ...
        >
          <NuxtLink
            :to="post.path"
            class="flex items-center px-6 py-4 group"
          >
              ...
              <div class="flex flex-col">
                ...
                <!-- <div class="text-sm text-gray-600 dark:text-gray-300 mt-1">
                                    <span v-if="post.authors && post.authors.length > 0">
                                        {{ post.authors ? post.authors.join(', ') : post.authors }}
                                    </span>
                                </div> -->
              </div>
            </div>
