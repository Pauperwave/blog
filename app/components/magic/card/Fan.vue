...
interface Props {
  cards: string[]
  caption?: string
  /** Total rotation arch in degrees, matches magic.wizards.com's `overall-arch` (default: 20.5) */
  arch?: number
  /** Layout style, matches magic.wizards.com's `config` values: 'fan' (default) or 'hand' */
  layout?: 'fan' | 'hand'
}
...
<template>
  <figure class="my-8">
    <!-- Mobile: plain horizontal scroll strip, rotated cards overlapping don't work on narrow viewports -->
    <div class="md:hidden flex gap-3 overflow-x-auto px-4 pb-2 mask-[linear-gradient(to_right,transparent,black_5%,black_95%,transparent)]">
      <div
        v-for="item in cardsWithData"
        :key="item.name"
        class="shrink-0"
      >
        <UAlert
    ...
    <!-- Desktop/tablet: fanned arc. Cards are all absolutely stacked at the exact same
    center position (no margin/flex-based overlap) — the entire spread comes purely from
    rotating each one around a single shared pivot far below the fan, matching how
    magic.wizards.com's fan actually works (verified: every card's un-rotated `left` is
    identical there, it's rotation alone that fans them out). -->
    <div
